...
var defaultGetUrlAttributes = function defaultGetUrlAttributes(route, _ref) {
  ...
  var loc = _route$sitemap.loc,
      rest = _objectWithoutProperties(_route$sitemap, ["loc"]);
...
function _main() {
  ...
  /*#__PURE__*/
  regeneratorRuntime.mark(function _callee(state, options) {
    var _state$config, DIST, disableRoutePrefixing, siteRoot, staging, prefixPath, filename, xml;

    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _state$config = state.config, DIST = _state$config.paths.DIST, disableRoutePrefixing = _state$config.disableRoutePrefixing, siteRoot = _state$config.siteRoot, staging = state.staging;
            prefixPath = disableRoutePrefixing ? siteRoot : process.env.REACT_STATIC_PUBLIC_PATH;
...
function generateXML(state) {
  ...
  .map(function (route) {
    var attributes = _objectSpread({}, defaultGetUrlAttributes(route, {
      config: config,
    ...
    return ['<url>'].concat(_toConsumableArray(attributesArr.map(function (_ref3) {
      var key = _ref3.key,
          value = _ref3.value;
      return "<".concat(key, ">").concat(encode(value), "</").concat(key, ">");
    })), ['</url>']).join('\n');
  }).join('\n');
